...
```bash
# 1. Docker (тільки якщо не встановлено)
./install_docker.sh

# 2. Налаштувати .env
cp .env.example .env
# відредагувати TELEGRAM_BOT_TOKEN, TOGETHER_API_KEY, ADMIN_TELEGRAM_ID

# 3. Все одною командою (build + up + migrations + redeploy)
./start.sh
```
...
Альтернатива деплою flows: відкрити http://localhost:1881, Menu -> Import -> `flows.json` -> Deploy.

Після підняття стеку — написати боту `/start` у Telegram.
...
├── install_docker.sh       Docker Engine + Compose plugin (Ubuntu/Debian)
├── start.sh                bootstrap: up + migrations + redeploy
├── apply_migrations.sh     bash
├── apply_migrations.ps1    powershell
├── redeploy.sh             bash
...
ER-діаграма: https://dbdiagram.io/d/69ef6f63ddb9320fdc66b2b2 (джерело: `db/dbdiagram.dbml`)
